    ...
    <div style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", background: '#09080A', minHeight: '100vh', color: '#DDD5C8', display: 'flex', flexDirection: 'column' }}>
      <style>{`
        @media (max-width: 768px) {
          .agent-layout { flex-direction: column !important; }
          .goal-sidebar { width: 100% !important; border-left: none !important; border-top: 1px solid #1A1820; }
          .goal-sidebar .goal-grid { display: grid !important; grid-template-columns: 1fr 1fr; gap: 10px !important; }
        }
      ...
      <div className="agent-layout" style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        {/* Main Content — Left */}
        ...
        {/* 90-Day Goal Tracker — Right Sidebar */}
        <div className="goal-sidebar" style={{ width: 260, flexShrink: 0, borderLeft: '1px solid #1A1820', background: '#0B0A0E', overflowY: 'auto', padding: '20px 16px' }}>
          <div style={{ fontSize: 10, letterSpacing: '0.2em', color: ac, textTransform: 'uppercase', marginBottom: 6 }}>90-Day Goals</div>
          {(() => {
            const fields = [
            ...
            return (
              <div style={{ background: '#0D0C10', border: '1px solid #1A1820', borderRadius: 12, padding: '12px', marginBottom: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                  <span style={{ fontSize: 10, color: '#5A5060', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Overall</span>
                  <span style={{ fontSize: 16, color: overallPct >= 100 ? '#6BAE94' : ac, fontWeight: 800 }}>{overallPct}%</span>
                </div>
                <div style={{ width: '100%', height: 8, background: '#1A1820', borderRadius: 4, overflow: 'hidden' }}>
          ...
          <div className="goal-grid" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {[
              ...
              return (
                <div key={f.key} style={{ background: '#0D0C10', border: `1px solid ${goalDone ? '#6BAE9430' : '#1A1820'}`, borderRadius: 12, padding: '12px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    <span style={{ fontSize: 14 }}>{f.icon}</span>
                    <span style={{ fontSize: 11, color: '#B8B0A8', flex: 1 }}>{f.label}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 8 }}>
                    <span style={{ fontSize: 24, color: goalDone ? '#6BAE94' : '#EEE5D5', fontWeight: 800 }}>{total}</span>
                    <span style={{ fontSize: 11, color: '#3A3040' }}>/ {f.goal}</span>
                    {goalDone && <span style={{ fontSize: 13, marginLeft: 'auto' }}>✅</span>}
                  </div>
                  <input
                  ...
                  />
                  <div style={{ fontSize: 9, color: '#3A3040', marginTop: 4, textAlign: 'right' }}>{Math.round(goalPct)}%</div>
                </div>
              );
            })}
          </div>
